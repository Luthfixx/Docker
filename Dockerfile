...
ADD https://raw.githubusercontent.com/Luthfixx/PocongUserbot/main/requirements.txt requirements.txt
RUN set -ex \
    && apt-get -qq update \
...
RUN curl https://raw.githubusercontent.com/creationix/nvm/v0.30.1/install.sh | bash     && . $NVM_DIR/nvm.sh     && nvm install $NODE_VERSION     && nvm alias default $NODE_VERSION     && nvm use default # buildkit

ENV NODE_PATH=/usr/local/nvm/v16.6.0/lib/node_modules

ENV PATH=/usr/local/nvm/versions/node/v16.6.0/bin:/usr/local/bin:/usr/local
